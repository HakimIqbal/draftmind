...
export function Topbar({ hasWorkspace = false, onToggleSidebar }: TopbarProps) {
  ...
  // Focus mobile search input when dialog opens
  useEffect(() => {
    if (mobileSearchOpen) {
  ...
  return (
    <>
      <header className="flex h-14 shrink-0 items-center justify-between gap-2 border-b border-[#f0f0ee] bg-white px-3 md:px-6">
        ...
        {/* Mobile: search icon — opens full-width search bar */}
        <button
          type="button"
      ...
      {/* Mobile search overlay — full-width bar at top */}
      {mobileSearchOpen && (
        <div className="fixed inset-0 z-50 bg-white md:hidden">
          <div className="flex h-14 shrink-0 items-center gap-2 border-b border-[#f0f0ee] px-3">
            <button
              type="button"
              onClick={() => setMobileSearchOpen(false)}
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[#666] hover:bg-[#f5f5f4] hover:text-[#1a1a1a]"
          ...
          <div className="flex-1 overflow-auto p-4">
            <p className="text-center text-[13px] text-[#aaa]">Type to search…</p>
          </div>
        </div>
      )}
    </>
