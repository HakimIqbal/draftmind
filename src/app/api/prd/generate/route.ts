...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      // If error is about no provider configured, use mock
      const errMsg = providerError instanceof Error ? providerError.message : String(providerError);
      if (errMsg.includes('No AI provider configured') || errMsg.includes('Provider not found')) {
        aiSections = generateMockPRDContent(inputPayload.title, inputPayload.brief);
        isMock = true;
        modelUsed = 'mock';
      } else {
        // Real error from AI call
